...
import { Clock, ArrowLeft, CheckCircle, AlertCircle, Loader, Headphones, Volume2, Play, Pause, Square, Radio } from 'lucide-react';
import { listeningApi, ListeningPart4 as ListeningPart4Type } from '../services/api';
...
const ListeningPart4: React.FC<ListeningPart4Props> = ({ onBackToDashboard }) => {
  const [task, setTask] = useState<ListeningPart4Type | null>(null);
  ...
  const [currentPhase, setCurrentPhase] = useState<'news' | 'questions'>('news');
  const [hasPlayedNews, setHasPlayedNews] = useState(false);
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const [newsCompleted, setNewsCompleted] = useState(false);
  ...
  const loadTask = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await listeningApi.generatePart4();
  ...
  const playNews = async () => {
    ...
    try {
      ...
      await ttsService.speak(task.news_item.content, {
        rate: 0.9,
        pitch: 1.0,
        volume: 0.8
      }, {
        ...
        onEnd: () => {
          setIsPlaying(false);
          setHasPlayedNews(true);
          setNewsCompleted(true);
        },
        onError: (error) => {
  ...
  // Remove TTS for questions - Part 4 uses visual-only questions
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="bg-white rounded-xl shadow-lg p-8 max-w-md mx-auto">
          ...
          <h2 className="text-xl font-bold text-center text-gray-800 mb-2">
            Generating Listening Part 4
          </h2>
          <p className="text-gray-600 text-center">
            Creating a local news item for you...
          </p>
        </div>
      </div>
    );
            ...
            <div className="flex items-center">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-800">
                  CELPIP Listening Part 4
                </h1>
                <p className="text-gray-600">Listening to News Item</p>
              </div>
        ...
        {showResults ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                <div className="space-y-4">
                  <div className={`border rounded-lg p-4 transition-all ${
                    hasPlayedNews ? 'border-green-300 bg-green-50' : 'border-blue-300 bg-blue-50'
                  }`}>
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="font-semibold text-gray-800">
                        {task.news_item.title}
                      </h4>
                        ...
                        {ttsSupported && (
                          <>
                            ...
                            <button
                              ...
                              disabled={!ttsSupported || hasPlayedNews}
                              className={`p-2 rounded-full transition-colors ${
                                !ttsSupported || hasPlayedNews
                                  ? 'bg-gray-400 cursor-not-allowed' 
                                  : 'bg-blue-600 hover:bg-blue-700'
                              } text-white`}
                              title={
                                !ttsSupported ? 'Text-to-Speech not supported' : 
                                hasPlayedNews ? 'Already played - can only listen once' :
                                'Play news item'
                              }
                            >
                              {isPlaying && !isPaused ? (
                    ...
                    <div className="text-xs text-gray-500 mb-2">
                      <div>Duration: {task.news_item.duration_seconds}s</div>
                      <div>Reporter: {task.news_item.reporter}</div>
                      <div>Location: {task.news_item.location}</div>
                      <div>Date: {task.news_item.date}</div>
                    </div>
                ...
                {ttsSupported && !isPlaying && !isPaused && (
                  <div className="mt-6 p-4 bg-green-50 rounded-lg border border-green-200">
                    ...
                    <div className="text-sm text-green-700 space-y-2">
                      <p>• Listen to the local news item carefully</p>
                      <p>• You can only listen to it ONCE</p>
                      <p>• Answer 5 questions after the news broadcast</p>
                ...
                {!ttsSupported && (
                  <div className="mt-6 p-4 bg-orange-50 rounded-lg border border-orange-200">
                    ...
                    <p className="text-sm text-orange-700">
                      Text-to-Speech is not supported in your browser. You can still read the news content 
                ...
                <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                  <h4 className="font-semibold text-gray-800 mb-2">News Details</h4>
                  <p className="text-sm text-gray-700 mb-3">
                    {task.news_item.audio_description}
                  </p>
                  
                  <h4 className="font-semibold text-gray-800 mb-2">Topic</h4>
                  <p className="text-sm text-gray-700">
                    {task.news_item.topic}
                  </p>
                  ...
                  {(process.env.NODE_ENV === 'development' || !ttsSupported) && (
                    <div className="mt-4">
                      <h4 className="font-semibold text-gray-800 mb-2">
                        News Content {process.env.NODE_ENV === 'development' ? '(Dev Mode)' : '(Audio Alternative)'}
                      </h4>
                      <div className="bg-white p-3 rounded border max-h-40 overflow-y-auto">
                        <div className="tts-transcript text-gray-700">
                          {task.news_item.content}
                        </div>
                      </div>
                    </div>
                  )}
            ...
            <div className="lg:col-span-2">
              <div className="bg-white rounded-xl shadow-lg p-6">
                {currentPhase === 'news' || isPlaying || !hasPlayedNews ? (
                  // News Phase Instructions
                  <div className="text-center py-12">
                    <div className="mb-6">
                      ...
                      <h3 className="text-xl font-bold text-gray-800 mb-2">
                        Listen to the Local News Item
                      </h3>
                      <p className="text-gray-600">
                        {isPlaying ? 'Listening to news broadcast...' : 
                         hasPlayedNews ? 'News broadcast completed!' : 
                         'Click the play button to hear the local news item. You can only listen once!'}
                      </p>
                    </div>
                    ...
                    {isPlaying ? (
                      ...
                      <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                        ...
                        <p className="text-green-800 font-medium mb-4">
                          News broadcast completed! Now answer the questions.
                        </p>
                        <button
                      ...
                      <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                        <p className="text-blue-800 mb-4">
                          Press play to begin the local news broadcast.
                        </p>
                        <p className="text-sm text-blue-600">
                          Remember: You can only listen to the news item once during the test.
                        </p>
                      </div>
                    )}
                  </div>
