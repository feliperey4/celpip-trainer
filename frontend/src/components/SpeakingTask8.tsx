...
import { speakingApi, SpeakingTask8 as SpeakingTask8Data, SpeakingTask8Score } from '../services/api';
...
const SpeakingTask8: React.FC<SpeakingTask8Props> = ({ onBackToDashboard }) => {
  const [task, setTask] = useState<SpeakingTask8Data | null>(null);
  ...
  // Results state
  const [score, setScore] = useState<SpeakingTask8Score | null>(null);
  ...
  useEffect(() => {
    const fetchTask = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await speakingApi.generateTask8();
  ...
  const submitRecording = async () => {
    ...
    try {
      const response = await speakingApi.scoreTask8({
    ...
    return (
      <div className="min-h-screen celpip-gradient-bg p-4 flex items-center justify-center">
        <div className="celpip-card p-8 text-center">
          <Loader className="animate-spin mx-auto mb-4 h-8 w-8 text-purple-500" />
      ...
      <div className="min-h-screen celpip-gradient-bg p-4 flex items-center justify-center">
        <div className="celpip-card p-8 text-center max-w-md">
          ...
          <div className="space-y-2">
            <button
              onClick={retryTask}
              className="w-full bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 text-white px-4 py-2 rounded-lg font-semibold transition-all"
            >
              Try Again
            </button>
            <button
        ...
        <div className="max-w-4xl mx-auto px-4">
          <div className="celpip-card p-8 animate-fade-in">
            <div className="text-center mb-8">
              ...
              <p className="text-lg text-gray-600 text-left">Your CELPIP Speaking Task 8 Results</p>
            </div>
    ...
    <div className="min-h-screen celpip-gradient-bg py-8">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-800 mb-2">CELPIP Speaking Task 8</h1>
            <p className="text-gray-600 text-left">Describing an Unusual Situation</p>
          </div>
          <button
            onClick={onBackToDashboard}
        ...
        {timer.isActive && (
          ...
          {currentPhase === 'instructions' && (
            ...
            <div className="text-center mb-8">
              <h2 className="text-2xl font-bold text-gray-900 mb-4">Task 8: Describing an Unusual Situation</h2>
              <p className="text-left text-gray-600 text-lg leading-relaxed">
                You will look at a picture of an unusual situation and describe what you see. Explain what makes it unusual and suggest possible explanations for why the situation might be happening.
              </p>
            </div>
            
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-6 mb-8">
              <h3 className="font-semibold text-purple-800 mb-3">Task Instructions:</h3>
              <ul className="space-y-2 text-purple-700">
                <li className="flex items-start">
                  <span className="w-2 h-2 bg-purple-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                  <strong>Preparation time:</strong> 30 seconds to observe and think about explanations
                </li>
                <li className="flex items-start">
                  <span className="w-2 h-2 bg-purple-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                  <strong>Speaking time:</strong> 60 seconds to describe the unusual situation
                </li>
                <li className="flex items-start">
                  <span className="w-2 h-2 bg-purple-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                  <strong>Goal:</strong> Describe the situation and offer creative explanations
                </li>
              </ul>
            </div>
            ...
            <div className="text-center">
              <button
                ...
                className="bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 disabled:from-purple-400 disabled:to-purple-500 text-white px-8 py-3 rounded-lg font-semibold shadow-lg hover:shadow-xl transition-all transform hover:scale-105 disabled:cursor-not-allowed"
              >
                Begin Task
              </button>
            </div>
          ...
          {currentPhase === 'preparation' && (
            <div className="space-y-6">
              <div className="text-center mb-8">
                ...
                <p className="text-gray-600">Study the unusual situation and think about possible explanations. You have 30 seconds.</p>
              </div>
              <div className="bg-purple-50 p-6 rounded-xl border border-purple-200">
                <h3 className="text-xl font-bold text-purple-800 mb-4">{task.scenario.title}</h3>
                
                {/* Display the generated situation image */}
                {task.situation_image ? (
                  <div className="bg-white rounded-lg p-4 shadow-sm mb-4">
                    <h4 className="font-medium text-gray-700 mb-3">Unusual Situation to Describe:</h4>
                    <div className="flex justify-center">
                      <img 
                        src={`data:image/jpeg;base64,${task.situation_image}`}
                  ...
                  <div className="bg-white rounded-lg p-6 shadow-sm">
                    <h4 className="font-medium text-gray-700 mb-3">Situation Description:</h4>
                    <p className="text-gray-600 leading-relaxed whitespace-pre-wrap text-left">
                      {task.scenario.situation_description}
                    </p>
                  </div>
                )}
                
                {task.scenario.unusual_elements && task.scenario.unusual_elements.length > 0 && (
                  <div className="mt-4 bg-purple-50 rounded-lg p-4">
                    <h4 className="font-medium text-purple-800 mb-2">Unusual Elements to Notice:</h4>
                    <ul className="text-sm text-purple-700 space-y-1">
                      {task.scenario.unusual_elements.map((element, index) => (
              ...
              <div className="bg-blue-50 p-6 rounded-xl border border-blue-200">
                ...
                <ul className="space-y-2">
                  <li className="flex items-start">
                    ...
                    <span className="text-gray-700 text-left">What you see in the picture</span>
                  </li>
                  <li className="flex items-start">
                    <div className="w-2 h-2 bg-blue-500 rounded-full mt-2 mr-3 flex-shrink-0"></div>
                    <span className="text-gray-700 text-left">What makes this situation unusual or strange</span>
                  </li>
                  <li className="flex items-start">
                    <div className="w-2 h-2 bg-blue-500 rounded-full mt-2 mr-3 flex-shrink-0"></div>
                    <span className="text-gray-700 text-left">Possible explanations for why this is happening</span>
                  </li>
                  <li className="flex items-start">
                    <div className="w-2 h-2 bg-blue-500 rounded-full mt-2 mr-3 flex-shrink-0"></div>
                    <span className="text-gray-700 text-left">Creative but logical reasons for the situation</span>
                  </li>
                </ul>
              </div>
          ...
          {currentPhase === 'speaking' && (
            <div className="space-y-6">
              <div className="text-center">
                ...
                <p className="text-gray-600 text-left">Describe the unusual situation and offer possible explanations</p>
              </div>
              ...
              <div className="bg-purple-50 p-6 rounded-xl border border-purple-200">
                ...
                {task.situation_image ? (
                  ...
                  <p className="text-gray-700 text-left mb-3">{task.scenario.situation_description}</p>
                )}
                
                {task.scenario.descriptive_focus && (
                  <p className="text-purple-600 text-sm text-left">
                    <strong>Focus:</strong> {task.scenario.descriptive_focus}
                  </p>
                )}
              </div>
