...
import { Clock, ArrowLeft, CheckCircle, AlertCircle, Loader, Volume2, Play, Pause, Square, MessageSquare, User } from 'lucide-react';
import { listeningApi, ListeningPart6 as ListeningPart6Type } from '../services/api';
...
const ListeningPart6: React.FC<ListeningPart6Props> = ({ onBackToDashboard }) => {
  const [task, setTask] = useState<ListeningPart6Type | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [userAnswers, setUserAnswers] = useState<UserAnswers>({});
  const [timeRemaining, setTimeRemaining] = useState(8 * 60); // 8 minutes for questions
  ...
  const [currentPhase, setCurrentPhase] = useState<'viewpoint' | 'questions'>('viewpoint');
  const [hasListenedViewpoint, setHasListenedViewpoint] = useState(false);
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const [viewpointCompleted, setViewpointCompleted] = useState(false);

  // Timer effect - only starts after viewpoint is heard
  useEffect(() => {
    if (timeRemaining > 0 && !isCompleted && hasListenedViewpoint && currentPhase === 'questions') {
      const timer = setTimeout(() => {
        setTimeRemaining(timeRemaining - 1);
      }, 1000);
      return () => clearTimeout(timer);
    } else if (timeRemaining === 0 && hasListenedViewpoint) {
      handleSubmit();
    }
  }, [timeRemaining, isCompleted, hasListenedViewpoint, currentPhase]);
  ...
  const loadTask = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await listeningApi.generatePart6();
  ...
  const playViewpointPresentation = async () => {
    ...
    try {
      ...
      await ttsService.speak(task.viewpoint.content, {
        rate: 0.9,
        pitch: 1.0,
        volume: 0.8
      }, {
        ...
        onEnd: () => {
          setIsPlaying(false);
          setHasListenedViewpoint(true);
          setViewpointCompleted(true);
        },
        onError: (error) => {
      ...
      console.error('Error playing viewpoint presentation:', error);
      setIsPlaying(false);
    }
  };
  ...
  const togglePlayback = () => {
    if (isPaused) {
      ...
      playViewpointPresentation();
    }
  };

  const proceedToQuestions = () => {
    if (hasListenedViewpoint) {
      setCurrentPhase('questions');
      setTimeRemaining(8 * 60); // Start 8-minute timer for questions
    }
  };
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="bg-white rounded-xl shadow-lg p-8 max-w-md mx-auto">
          ...
          <h2 className="text-xl font-bold text-center text-gray-800 mb-2">
            Generating Listening Part 6
          </h2>
          <p className="text-gray-600 text-center">
            Creating a viewpoint presentation for you...
          </p>
        </div>
      </div>
    );
            ...
            <div className="flex items-center">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-800">
                  CELPIP Listening Part 6
                </h1>
                <p className="text-gray-600">Listening to Viewpoints</p>
              </div>
        ...
        {showResults ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                <div className="space-y-4">
                  <div className={`border rounded-lg p-4 transition-all ${
                    hasListenedViewpoint ? 'border-green-300 bg-green-50' : 'border-blue-300 bg-blue-50'
                  }`}>
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="font-semibold text-gray-800">
                        {task.viewpoint.title}
                      </h4>
                      <div className="flex items-center gap-2">
                        {hasListenedViewpoint && (
                        ...
                        {ttsSupported && (
                          <>
                            ...
                            <button
                              ...
                              disabled={!ttsSupported || hasListenedViewpoint}
                              className={`p-2 rounded-full transition-colors ${
                                !ttsSupported || hasListenedViewpoint
                                  ? 'bg-gray-400 cursor-not-allowed' 
                                  : 'bg-blue-600 hover:bg-blue-700'
                              } text-white`}
                              title={
                                !ttsSupported ? 'Text-to-Speech not supported' : 
                                hasListenedViewpoint ? 'Already heard - can only listen once' :
                                'Play viewpoint presentation'
                              }
                            >
                              {isPlaying && !isPaused ? (
                    ...
                    <div className="text-xs text-gray-500 mb-2">
                      <div>Duration: {task.viewpoint.duration_seconds}s</div>
                      <div>Speaker: {task.viewpoint.speaker}</div>
                      <div>Position: {task.viewpoint.position}</div>
                    </div>
                ...
                {ttsSupported && !isPlaying && !isPaused && (
                  <div className="mt-6 p-4 bg-green-50 rounded-lg border border-green-200">
                    ...
                    <div className="text-sm text-green-700 space-y-2">
                      <p>• Listen to the viewpoint presentation carefully</p>
                      <p>• You can only listen to it ONCE</p>
                      <p>• Answer 6 questions after the presentation</p>
                      <p>• You have 8 minutes to complete the questions</p>
                    </div>
                  </div>
                )}
                ...
                {!ttsSupported && (
                  <div className="mt-6 p-4 bg-orange-50 rounded-lg border border-orange-200">
                    ...
                    <p className="text-sm text-orange-700">
                      Audio simulation is not supported in your browser. You can still read the viewpoint content 
                ...
                <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                  <h4 className="font-semibold text-gray-800 mb-2">Presentation Details</h4>
                  <p className="text-sm text-gray-700 mb-3">
                    {task.viewpoint.audio_description}
                  </p>
                  
                  <h4 className="font-semibold text-gray-800 mb-2">Key Arguments</h4>
                  <ul className="text-sm text-gray-700 space-y-1 mb-3">
                    {task.viewpoint.key_arguments.map((argument, index) => (
                  ...
                  <h4 className="font-semibold text-gray-800 mb-2">Supporting Evidence</h4>
                  <ul className="text-sm text-gray-700 space-y-1">
                    {task.viewpoint.supporting_evidence.map((evidence, index) => (
                      <li key={index} className="flex items-start">
                  ...
                  {(process.env.NODE_ENV === 'development' || !ttsSupported) && (
                    <div className="mt-4">
                      <h4 className="font-semibold text-gray-800 mb-2">
                        Presentation Content {process.env.NODE_ENV === 'development' ? '(Dev Mode)' : '(Audio Alternative)'}
                      </h4>
                      <div className="bg-white p-3 rounded border max-h-40 overflow-y-auto">
                        <div className="tts-transcript text-gray-700 whitespace-pre-line">
                          {task.viewpoint.content}
                        </div>
                      </div>
                    </div>
                  )}
            ...
            <div className="lg:col-span-2">
              <div className="bg-white rounded-xl shadow-lg p-6">
                {currentPhase === 'viewpoint' || isPlaying || !hasListenedViewpoint ? (
                  // Viewpoint Phase Instructions
                  <div className="text-center py-12">
                    <div className="mb-6">
                      ...
                      <h3 className="text-xl font-bold text-gray-800 mb-2">
                        Listen to the Viewpoint Presentation
                      </h3>
                      <p className="text-gray-600">
                        {isPlaying ? 'Listening to viewpoint presentation...' : 
                         hasListenedViewpoint ? 'Presentation completed!' : 
                         'Click the play button to hear the viewpoint presentation. You can only listen once!'}
                      </p>
                    </div>
                    ...
                    {isPlaying ? (
                      <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
                        <div className="flex items-center justify-center mb-2">
                          ...
                          <span className="text-yellow-800 font-medium">
                            Playing Viewpoint Presentation...
                          </span>
                        </div>
                        <p className="text-sm text-yellow-700">
                          Listen carefully to the speaker's arguments and evidence!
                        </p>
                      </div>
                    ) : hasListenedViewpoint ? (
                      <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                        ...
                        <p className="text-green-800 font-medium mb-4">
                          Presentation completed! Now answer the questions.
                        </p>
                        <button
                      ...
                      <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                        <p className="text-blue-800 mb-4">
                          Press play to begin the viewpoint presentation.
                        </p>
                        <p className="text-sm text-blue-600">
                          Remember: You can only listen to the presentation once during the test.
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-lg font-bold text-gray-800">
                        Questions for Viewpoint Presentation
                      </h3>
